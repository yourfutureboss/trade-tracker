...
def last_marks(ticker, n=2):
    return sb("price_marks", ticker=f"eq.{ticker}",
              select="mark_date,price", order="mark_date.desc", limit=n)

MOVE_GATE = 0.25  # corporate-action sanity: suppress level alerts on >25% d/d moves
...
def main():
    ...
    for s in setups:
        ...
        prev = float(mm[1]["price"]) if len(mm) > 1 else None
        gated = prev is not None and prev > 0 and abs(px / prev - 1) > MOVE_GATE
        if gated:
            hits.append((f"[data] {t} moved {abs(px/prev-1)*100:.0f}% day-over-day",
                         f"{t}: {prev} -> {px} ({mm[1]['mark_date']} -> {m['mark_date']}). "
